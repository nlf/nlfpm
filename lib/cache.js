...
Cache.prototype._cache_search = function (key, request, response, reply) {
    ...
    wreck.read(response, {}, function (err, body) {
...
Cache.prototype.putHandler = function (request, reply) {
    ...
    wreck.put(url, { headers: headers, payload: payload, json: true }, function (err, res, payload) {
        if (err) {
            return reply(err);
        }
...
Cache.prototype.primeSearch = function (plugin, callback) {
    ...
    cache.client.exists('search', function (err, exists) {
        ...
        wreck.get('https://' + cache.registries.public + '/-/all', { json: true }, function (err, res, parsed) {
            if (err) {
                throw err;
            }

            cache.client.hset('search', parsed._updated, JSON.stringify(parsed), function (err, result) {
                if (err) {
                    throw err;
                }
